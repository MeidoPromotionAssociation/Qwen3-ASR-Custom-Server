...
- 客户端可以逐请求设置 `convert_audio=true`，让服务端用 FFmpeg 把上传内容转换为 16 kHz 单声道 PCM WAV；该选项默认是 `false`，已经自行转换音频的客户端无需重复处理。
...
- FFmpeg（仅在客户端使用 `convert_audio=true` 时需要）
...
客户端需要把 MPEG、MP3、AAC、M4A、WebM、Ogg/Opus 等输入交给服务端转换时，应先安装 FFmpeg，并确保运行服务的用户能从 `PATH` 找到 `ffmpeg`：
...
Windows 可以安装一个包含 `ffmpeg.exe` 的发行版，然后执行：

```powershell
ffmpeg -version
```

如果可执行文件不在 `PATH`，通过 `QWEN_ASR_FFMPEG` 指定完整路径。FFmpeg 并非服务启动的硬依赖：找不到它时模型仍会加载，`convert_audio=false` 的请求仍可工作；只有显式请求 `convert_audio=true` 时返回 HTTP 503。
...
- `convert_audio` 使用相同的布尔写法，默认 `false`。设置为 `true` 时，服务端先用 FFmpeg 选择第一个音轨，再统一转换为 16 kHz、单声道、16-bit PCM WAV；设置为 `false` 或省略时，原上传内容直接交给模型音频后端。
- 批量接口中的一个 `convert_audio` 值控制本次请求的全部文件，不支持逐文件混用转换策略；需要混用时拆成多个请求。
- 服务端可转换的输入格式和编码取决于实际安装的 FFmpeg build。损坏文件、没有音轨或当前 FFmpeg 不含相应解码器时返回 HTTP 400。
...
| `audio_conversion_default` | boolean | `convert_audio` 的默认值，当前固定为 `false` |
| `audio_conversion_available` | boolean | 启动时是否找到了 `QWEN_ASR_FFMPEG` 指定的可执行文件 |
| `audio_sample_rate` | integer | 服务端转换后的采样率，当前为 16000 Hz |
| `ffmpeg_timeout_seconds` | integer | 单个文件的 FFmpeg 转换超时秒数 |
...
| `convert_audio` | boolean string | 否 | `false` | 是否先由服务端 FFmpeg 转为 16 kHz 单声道 PCM WAV |
...
```bash
curl -X POST http://127.0.0.1:8000/v1/audio/transcriptions \
  -F "file=@./recording.mpeg" \
  -F "convert_audio=true" \
  -F "language=zh"
```
...
| `convert_audio` | boolean string | 否 | 0 或 1 | 是否由 FFmpeg 转换本批次的全部文件；默认 `false` |
...
如果这一批文件需要服务端转换，在同一个请求中再增加 `-F "convert_audio=true"`；该开关会应用到本批全部文件。
...
| `convert_audio` | boolean string | 否 | 是否先由服务端 FFmpeg 转换；默认 `false` |
...
| 状态码 | 场景 | 示例 |
|---:|---|---|
| `400` | 参数值非法、语言不支持、batch 字段数量不匹配、未启用 Aligner、FFmpeg 无法解码，或关闭转换时模型后端无法读取音频 | `{"detail":"The uploaded audio could not be decoded without server-side conversion. Retry with convert_audio=true or upload an audio format supported by the model backend."}` |
| `408` | 单个文件的 FFmpeg 转换超过 `QWEN_ASR_FFMPEG_TIMEOUT` | `{"detail":"Server-side audio conversion timed out after 120 seconds for 'song.mpeg'."}` |
...
| `503` | 服务或模型尚未就绪，或请求了转换但 FFmpeg 不可用 | `{"detail":"Server-side audio conversion is unavailable because ffmpeg was not found. Install ffmpeg or retry with convert_audio=false for an already supported audio file."}` |
...
默认的 `convert_audio=false` 路径无法读取某个格式时，不再让底层 `audioread.NoBackendError` 直接形成未处理的 500，而是返回：

```json
{
  "detail": "The uploaded audio could not be decoded without server-side conversion. Retry with convert_audio=true or upload an audio format supported by the model backend."
}
```

此时客户端可以用同一个文件重试并增加 `convert_audio=true`。如果 FFmpeg 也无法解码，HTTP 400 的 `detail` 会包含经过长度限制的 FFmpeg 错误摘要，便于判断文件损坏、没有音轨或缺少解码器；服务端临时路径会从摘要中隐藏。
...
1. 如果客户端会提交 `convert_audio=true`，先安装 FFmpeg，并确认 systemd 服务用户能执行它。
2. 复制到 `/etc/systemd/system/qwen3-asr.service`。
3. 按实际路径修改 `User`、`WorkingDirectory` 和 `ExecStart`。
4. 不需要时间戳时，删除 `QWEN_ALIGNER_MODEL` 和 `QWEN_ALIGNER_DTYPE`。
5. 根据 CPU 核心数调整线程环境变量。
...
| `QWEN_ASR_FFMPEG` | `ffmpeg` | 服务端转换使用的 FFmpeg 命令名或完整路径；启动时解析一次 |
| `QWEN_ASR_FFMPEG_TIMEOUT` | `120` | FFmpeg 转换单个文件的超时秒数，必须大于 0 |
...
- 客户端已经生成模型可读的 WAV 时保持默认 `convert_audio=false`，避免重复有损解码和额外 I/O；原格式不受模型后端支持时再显式使用 `convert_audio=true`。
